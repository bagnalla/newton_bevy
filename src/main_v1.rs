...
fn collision_system(mut query: Query<(&mut Transform, &mut Velocity, &Radius, &Mass)>) {
    ...
    while let Some([(mut a_transform, mut a_velocity, Radius(a_radius), Mass(a_mass)),
		...
		*a_velocity =
		    Velocity(v1 - (2.0 * b_mass / (a_mass + b_mass)) *
			     ((v1 - v2).dot(v) / ds) * v);
		*b_velocity =
		    Velocity(v2 - (2.0 * a_mass / (a_mass + b_mass)) *
			     ((v2 - v1).dot(-v) / ds) * -v);
		let mass_ratio = b_mass / (a_mass + b_mass);
		a_transform.translation += dir * mass_ratio * d;
		b_transform.translation += -dir * (1.0 - mass_ratio) * d
	    }
	}
}
